...
export function PhaseCards({ phases }: Props) {
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      {phases.map((phase) => {
        // Handle null status
        const hasStatus = phase.status !== null && phase.status !== undefined;
        const statusConfig = hasStatus ? getStatusConfig(phase.status!) : null;
        ...
        // Determine if phase is completed or in transit
          ...
          <Card key={phase.id} className="hover:shadow-md transition-shadow">
            <CardHeader className="pb-3">
              <div className="space-y-3">
                ...
                {hasStatus && statusConfig && StatusIcon ? (
                  <Badge
                    variant="outline"
            ...
            <CardContent className="space-y-4">
              ...
              {hasStatus && statusConfig ? (
                ...
                <div className="p-3 rounded-md border border-muted bg-muted/50 text-muted-foreground">
                  <p className="text-xs">
                    Phase status will be updated by your admin once work begins.
                  </p>
                </div>
              )}
            </CardContent>
